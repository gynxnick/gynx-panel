...
class PluginInstallerService
{
    public function __construct(
        ...
        private PluginSourceRegistry $registry,
    ...
    public function search(Server $server, string $sourceSlug, string $query, ?string $gameVersion = null): array
    {
        ...
        $hits = $source->search($query, $gameVersion);
    ...
    public function install(
        ...
        $source = $this->registry->get($sourceSlug);
        if (!$source->available()) {
            throw new ConflictHttpException("Source '{$sourceSlug}' is not available.");
        }
        ...
        $dl = $source->resolveDownload($externalId, $versionId, $gameVersion);
